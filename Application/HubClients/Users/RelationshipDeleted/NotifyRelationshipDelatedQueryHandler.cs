...
namespace Sparkle.Application.HubClients.Users.RelationshipDeleted
{
    public class NotifyRelationshipDelatedQueryHandler : HubHandler, IRequestHandler<NotifyRelationshipDelatedQuery>
    {
        ...
        public async Task Handle(NotifyRelationshipDelatedQuery query, CancellationToken cancellationToken)
        {
            ...
            await SendAsync(ClientMethods.RelationshipsDeleted, modelForCurrentUser, GetConnectionsAsync(UserId));
            await SendAsync(ClientMethods.RelationshipsDeleted, modelForOtherUser, GetConnectionsAsync(notCurrentUserId));
        }
    }
}
